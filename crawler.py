...
class crawler:
	...
	def process(self, date, min_age, max_age, show_available, token):
		...
		try:
			base = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id={}&date=".format(self.district)
			if token is None:
				...
				base = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/calendarByDistrict?district_id={}&date=".format(self.district)

			url = "{0}{1}".format(base, date)
			log.debug(url)
